...
class TrackedMovie(models.Model):
    """追蹤電影（tracked_movie）— Phase 3 新增。

    這是後台第一張「自己擁有」的 managed 資料表（managed=True 為預設，
    會產生正式 migration），與上方 8 張唯讀對映舊 SQLite schema 的
    unmanaged models 不同。

    定位：取代專案根的《電影清單.txt》成為追蹤電影的真相來源；
    txt 降級為相容匯出（見 export_movie_list / import_movie_list 指令）。
    """
    ...
    class Meta:
        # 注意：不設 managed=False —— 這張表由 Django migration 建立與維護
