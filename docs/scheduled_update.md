...
2. **確認後台 Postgres 已有含代碼的影城主檔。** 影城據點的 `source_location_code`（各官網代碼）是爬蟲的關鍵；只有在本機對 Render Postgres 跑過 `import_from_sqlite` 才會帶入（`import_from_geojson` 不含代碼）。自查：

   ```bash
   cd backend
   DATABASE_URL="postgres://…render 外部連線字串…" python manage.py shell -c \
   "from mapdata.models import CinemaLocation as L; print('有代碼的據點數:', L.objects.exclude(source_location_code__isnull=True).exclude(source_location_code='').count())"
   ```

   若為 0，先在本機灌入（會以自然鍵 upsert，冪等）：

   ```bash
   cd backend
   DATABASE_URL="postgres://…render 外部連線字串…" python manage.py import_from_sqlite ../data/movie_map.sqlite
   ```
